...
## . or .. depends on the current commandline environment location. not the script location
## assume the commandline environment is at the rhessys folder under project

RscriptFolder = './R-coded-scripts-for-RHESSys-calibration-master/'
...
script_to_obs = paste('../obs/',arg[2],sep='')
script_to_output = paste(arg[7],sep='')
...
outputFolder = arg[7] ## need this for naming file purpose (below)
...
plotTime = intersectDate(list(rhessys.date, calobs.date0, period)) ## <<--- return daily series
rhessys.dailytimeSeriesMatch = match(plotTime, rhessys.date)
calobs.dailytimeSeriesMatch = match(plotTime, calobs.date0)
calobs.dailytimeSeries = dailyTimeSeries(plotTime)
